...
CollapsibleSection {
    ...
    // ---- Current wallpaper preview + folder hint ----
    SectionContainer {
        ...
        StyledClippingRect {
            ...
            CachingImage {
                anchors.fill: parent
                path: Thumbs.path(Wallpapers.actualCurrent)
        ...
        StyledText {
            Layout.fillWidth: true
            text: Wallpapers.actualCurrent ? Paths.shortenHome(Wallpapers.actualCurrent) : qsTr("(no wallpaper set)")
        ...
        StyledText {
            Layout.fillWidth: true
            text: qsTr("Drop wallpapers in: %1").arg(Paths.shortenHome(Paths.wallsdir))
            color: Colours.palette.m3onSurfaceVariant
            elide: Text.ElideMiddle
            wrapMode: Text.NoWrap
        }
