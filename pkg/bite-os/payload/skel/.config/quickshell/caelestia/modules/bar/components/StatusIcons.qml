...
StyledRect {
    ...
    // Low-battery watcher: fires a toast recommending Saver mode when the
    // battery drops past 20% / 10% / 5% while on battery. Each threshold only
    // fires once per shell session and rearms once you're charging again.
    Item {
        id: batteryWatcher
            ...
            function onPercentageChanged(): void {
                ...
                for (const t of thresholds) {
                    if (perc <= t && batteryWatcher.lastNotified > t) {
                        ...
                        const urgency = t <= 5 ? "critical" : (t <= 10 ? "critical" : "normal");
