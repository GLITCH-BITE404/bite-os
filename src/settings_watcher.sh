...
compile_settings() {
    ...
    # The line is first-boot-guarded: it shows the guide once, drops a marker file,
    # and never opens it again — so it onboards a fresh install without nagging.
    if [[ $(jq -r 'if (if type == "object" and has("openGuideAtStartup") then .openGuideAtStartup else true end) then "yes" else "no" end' "$SETTINGS_FILE") == "yes" ]]; then
        echo "exec-once = bash -c '[ -f ~/.local/state/bite-os/guide-shown ] || { sleep 1.5; ~/.config/hypr/scripts/qs_manager.sh toggle guide; mkdir -p ~/.local/state/bite-os; touch ~/.local/state/bite-os/guide-shown; }'" >> "$AUTOSTART_CONF"
    fi
